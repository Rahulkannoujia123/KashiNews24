...
const parseGNewsStories = (articles) => articles.map((article, index) => {
  ...
  return {
    ...
    category: detectCategory(`${title} ${article.description || ''}`),
    location: 'वाराणसी',
  ...
  try {
    ...
    if (apiKey) {
      const params = new URLSearchParams({
        q: `वाराणसी समाचार${hindiCategoryQueries[category.toLowerCase()] ? ` ${hindiCategoryQueries[category.toLowerCase()]}` : ''}`,
        lang: 'hi',
        country: 'in',
        max: '10',
        apikey: apiKey
      ...
      const items = parseGNewsStories(data.articles || []);
...
export const navItems = ['Home', 'Varanasi', 'Kashi', 'Crime', 'Politics', 'Traffic', 'Education', 'Jobs', 'Business', 'Sports', 'Weather', 'Events', 'Videos'];
